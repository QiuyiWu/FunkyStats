...
- Half of the stops involved were of white drivers, about four times the number of stops of black people
...
We want to investigate how likely contraband items are found when searching is performed. We argue that if racial bias does not exist, the hit rate should be equal for all races. In other words, we expect to find that `race` is not an essential factor in the model:
...
We proceed using `stan_glmer` and the default prior on the covariance matrix. 
